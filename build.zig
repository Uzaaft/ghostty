...
pub fn build(b: *std.Build) !void {
    ...
    // libghostty-vt. Native freestanding targets don't have a shared
    // library format, so we only install the static library below.
    ...
    if (config.is_dep) {
        ...
        // The shared artifact normally installs our headers. We don't have
        // one on freestanding targets, so install them with the static lib.
